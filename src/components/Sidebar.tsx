...
import { Badge, Container, Box, Stack, FormControl, FormLabel, FormGroup, FormControlLabel, Checkbox, InputLabel, OutlinedInput, MenuItem } from '@mui/material';
...
const Sidebar = (props: {
    ...
    return (
      keys.map((k) =>
        <MenuItem key={k} value={k}
          style={getStyles(k, Array.from(ctx.resourceTypes![kind].entries()).filter(kv => kv[1]).map(kv => kv[0]), theme)}
        > {k} </MenuItem>
      )
    )
  };

  const handleTypesChange = (kind: ResourceProparty, event: SelectChangeEvent<string[]>) => {
    const {
      target: { value },
    } = event;
    ...
    const not_selected = Array.from(ctx.resourceTypes![kind].keys()).filter((s) => selected.indexOf(s) === -1);
  ...
  const renderTypeSelectForm = (kind: ResourceProparty) => {
    return <FormControl sx={{ m: 1, width: 230 }}>
          <InputLabel id={"select-label-"+kind}>Select {kind}</InputLabel>
          <Select
            labelId={"select-label-"+kind}
            id={"select-"+kind}
            multiple
            value={Array.from(ctx.resourceTypes![kind].entries()).filter(kv=>kv[1]).map(kv=>kv[0])}
            onChange={handleTypesChange.bind(null, kind)}
            input={<OutlinedInput id={"select-"+kind} label={"resource-"+kind} />}
