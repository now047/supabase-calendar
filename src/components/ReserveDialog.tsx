  ...
  return (
    <Dialog onClose={handleClose.bind(null, true)} open={props.open}>
      ...
      <Box sx={{ '& > :not(style)': { m: 2 } }} width={300}>
        ...
        >
          <DemoItem label={<ProLabel>Purpose</ProLabel>} component="Purpose" >
            <TextField autoFocus required 
              error={purposeOfUseError}
              helperText={purposeOfUseError? "Please enter a value" : ""}
              focused={purposeOfUse === ""}  id="pou-input" label="Input purpose of use" variant="outlined"
              ...
              >
                {
                  props.resources.map(r =>
                    <MenuItem key={"resource-" + r.id} value={r.id}>
                      <Stack direction="row" justifyContent={'flex-start'} spacing={1}>
                        <Avatar sx={{
                          bgcolor: colorMap.get(r.display_color),
                          width: 24, height: 24
                        }}> {r.name[0]} </Avatar>
                        <Typography>
                          {r.name}
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DemoItem label={<ProLabel>Start Date & Time</ProLabel>} component="Start" >
              <MobileDateTimePicker value={start} onChange={handleStartChange}/>
            </DemoItem>
            <DemoItem label={<ProLabel>End Date & Time</ProLabel>} component="End" >
              <MobileDateTimePicker value={end} onChange={handleEndChange}/>
            </DemoItem>
